...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for cr in orm.ChangeRecord.objects.all():
            entry = cr.entry
            entry.user = cr.user
            entry.datetime = cr.datetime
            entry.session = cr.session
            entry.ctype = cr.ctype
            entry.csubtype = cr.csubtype
            entry.c_hash = cr.c_hash
            entry.save()

        orm.ChangeRecord.objects.all().delete()
