...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for entry in orm.Entry.objects.all():
            entry.deleted = (entry.latest.ctype == 'D')
            entry.save()

        for cr in orm.ChangeRecord.objects.all():
            if cr.ctype == "D":
                dsc = orm.DeletionChange(
                    entry=cr.entry,
                    ctype='D',
                    user=cr.user,
                ...
                dsc.save()
                # We must update latest and latest_published if necessary.
                entry = cr.entry
                if cr == entry.latest:
                    entry.latest = \
                        entry.changerecord_set.exclude(
                            id=cr.id).latest('datetime')
                if cr == entry.latest_published:
                    entry.latest_published = \
                        entry.changerecord_set.exclude(id=cr.id) \
                        .filter(published=True).latest('datetime')
                entry.save()
                cr.delete()

    def backwards(self, orm):
        raise Exception("can't go backwards")
