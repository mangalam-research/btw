        ...
        for data in itertools.chain(Entry.objects.all(),
            ...
            if chunk.pk not in chunk_mapping:
                ...
                with open(os.path.join(log_dir, chunk.pk, "before.xml"), 'w') \
                        as before:
                    before.write(storage.encode('utf-8'))

                if not util.validate(before_rng, storage):
                    self.stdout.write(
                ...
                converted = util.run_saxon(xsl_path, storage)
                new_chunk = Chunk()
                new_chunk.data = xml.storage_to_editable(converted)
