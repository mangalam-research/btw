...
""")
    ...
    def test_generate_monit_config_does_not_take_arguments(self):
        """
        Test that ``btwredis generate-monit-config`` does not take arguments.
        """
        self.check_no_arguments("generate-monit-config")

    def test_generate_monit_config_does_not_take_all(self):
        """
        Test that ``btwredis generate-monit-config`` does not take ``--all``.
        """
        self.check_no_all("generate-monit-config")

    def test_generate_monit_config_without_envpath(self):
        """
        Test that ``btwredis generate-monit-config`` generates correct
        output when ENVPATH is not set.
        """
        stdout, stderr = call_command("btwworker", "generate-monit-config")
...
testing.bibliography.worker"
            as uid btw and gid btw
      stop program = "python foo/manage.py btwworker stop \
testing.bibliography.worker"
            as uid btw and gid btw
      if does not exist then start
""")
    ...
    def test_generate_monit_config_with_envpath(self):
        """
        Test that ``btwredis generate-monit-config`` generates correct
        output when ENVPATH is set.
        """
...
testing.worker"
            as uid btw and gid btw
      stop program = "blah/bin/python foo/manage.py btwworker stop \
testing.worker"
            as uid btw and gid btw
      if does not exist then start
